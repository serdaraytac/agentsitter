...
const VAGUE_REPLACEMENTS: Array<{ pattern: RegExp; suggestion: string; skipInVcsContext?: boolean }> = [
...
function applyPattern(line: string, pattern: RegExp, suggestion: string): string {
  return line.replace(pattern, (...args: unknown[]) => {
    ...
    captures.forEach((cap, i) => {
      if (cap !== undefined) result = result.replace(new RegExp(`\\$${i + 1}`, "g"), cap);
    });
...
export function isReplacementSafe(replaced: string): boolean {
  if (/\[TODO:[^\]]*\[TODO:/.test(replaced)) return false;
  if ((replaced.match(/\[TODO:/g) ?? []).length >= 2) return false;
  return true;
}
...
export function fixVagueRules(content: string): { content: string; replacements: number; reverted: number } {
  ...
  const outputLines = inputLines.map((line) => {
    ...
    if (trimmed.includes("[TODO:")) return line;
    ...
    for (const { pattern, suggestion, skipInVcsContext } of VAGUE_REPLACEMENTS) {
      ...
      if (skipInVcsContext && VCS_CONTEXT_RE.test(line)) continue;
...
function addMissingSectionStubs(content: string, missingSections: string[], platform: string): { content: string; added: string[] } {
  ...
  for (const section of missingSections) {
    ...
    if (new RegExp(`<${escapedName}(?:\\s[^>]*)?>`, "i").test(content)) continue;
...
export function annotateCriticalRules(content: string): { content: string; blockCount: number } {
  const lines = content.split("\n");
  let insideBlock = false;
  let blockCount = 0;
  const result: string[] = [];
  let criticalBuffer: string[] = [];
  ...
  for (const line of lines) {
    ...
    if (insideBlock) { result.push(line); continue; }
...
export function moveCriticalRulesToTop(lines: string[], issues: Issue[]): { lines: string[]; moved: boolean } {
